<?php 
    ...
    function select_Propuestas($conn, $id, $status) {

        if ($_SESSION['user']['rol'] == 'gestor') {
            $sql = "SELECT p.id AS id, p.estatus AS estatus, p.nombre AS propuesta, CONCAT(t.nombre, ' ', t.apellidos) AS trabajador FROM propuestas AS p INNER JOIN trabajadores AS t ON p.id_trabajador = t.id WHERE p.id_proyecto = '$id' ORDER BY t.nombre";

            $propuesta = mysqli_query($conn, $sql);
        } else if ($_SESSION['user']['rol'] == 'diseñador') {
            if ($status == 'Produccion') {
                $sql = "SELECT nombre AS propuesta, id, estatus FROM propuestas  WHERE estatus = 'Aprobado'";
            } else {
                $id_trab = $_SESSION['user']['id'];
                $sql = "SELECT p.id AS id, p.estatus AS estatus, p.nombre AS propuesta, CONCAT(t.nombre, ' ', t.apellidos) AS trabajador FROM propuestas AS p INNER JOIN trabajadores AS t ON p.id_trabajador = t.id WHERE p.id_proyecto = '$id' AND p.id_trabajador = '$id_trab'";
            }
